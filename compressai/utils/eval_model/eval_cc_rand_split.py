...
@torch.no_grad()
def inference_entropy_estimation(model, x, slices):
    x = x.unsqueeze(0)

    start = time.time()
    out_net = model.forward(x, slices)
...
def gene_slices(slices, interval=40, num_slices=5, **args):
    if len(slices) == num_slices-1:
        print('slices:', slices)
        eval_model_specific_slices(slices, **args)
        return
    _min = slices[-1]+interval if slices else interval
    for i in range(_min, 320-interval*(num_slices-2-len(slices)), interval):
        gene_slices(slices+[i], interval, num_slices, **args)


def eval_model_specific_slices(slices, model, filepaths, device, entropy_estimation, half, recon_path, metrics):
    metric = defaultdict(float)
    slice_name = ','.join(map(str, slices)) + ','
    for f in filepaths:
        _filename = f.split("/")[-1]
        ...
        if not entropy_estimation:
            ...
            rv = inference_entropy_estimation(model, x, slices)
        for k, v in rv.items():
            metric[k] += v
    for k, v in metric.items():
        metric[k] = v / len(filepaths)
    metrics[slice_name] = metric
...
def main(argv):
    ...
    json.dump(output, open(os.path.join(args.recon_path, 'results.json'), 'w'), indent=4)
