...
def gene_table(num_slices=10, without_y=False, without_lrp=False):
    cc_table = {
            ...
            if without_lrp:
                cc_table['cc_scale_transforms.{}.4.compactor'.format(i)] = {
                    'weight': 'cc_scale_transforms.{}.4.conv.weight'.format(i), 'bias': 'cc_scale_transforms.{}.4.conv.bias'.format(i), 'suc_weight':
                        ['cc_scale_transforms.{}.0.conv.weight'.format(j) for j in range(i+1, num_slices)]
...
def pre_split_before_pruning(state_dict, num_slices, enhanced_resrep=False, without_y=False, without_lrp=False):
    if not without_y:
        # 只有g_a不太一样，分割第一维
...
def post_combine_after_pruning(state_dict, num_slices, without_y=False, without_lrp=False):
    if not without_y:
        # 只有g_a不太一样，分割第一维
        weights = []
        biases = []
